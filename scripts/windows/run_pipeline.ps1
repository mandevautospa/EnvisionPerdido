...
# Purpose: Activate venv, load env vars, run automated_pipeline.py, and log output
...
try {
    ...
    "[$(Get-Date -Format o)] Starting automated_pipeline.py" | Tee-Object -FilePath $logPath -Append

    $python = "python"
    & $python "scripts\automated_pipeline.py" 2>&1 | Tee-Object -FilePath $logPath -Append

    "[$(Get-Date -Format o)] Finished automated_pipeline.py" | Tee-Object -FilePath $logPath -Append
}
catch {
    try {
        if (-not $logPath) {
            ...
            $logPath = Join-Path $fallbackDir ("pipeline_" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".log")
        }
